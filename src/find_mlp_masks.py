...
def compute_mlps_importance(
    args, model, eval_dataloader, compute_importance=True, mlp_mask=None
):
    ...
    for step, batch in enumerate(tqdm(eval_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
        ...
        outputs = model(
            input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids, mlp_mask=mlp_mask
        )
    ...
    return mlp_importance, preds, labels
...
def mask_mlps(args, model, eval_dataloader):
    ...
    """
    mlp_importance, preds, labels = compute_mlps_importance(args, model, eval_dataloader)
...
def prune_heads(args, model, eval_dataloader, mlp_mask):
    ...
    _, preds, labels = compute_mlps_importance(
        args, model, eval_dataloader, compute_importance=False, mlp_mask=mlp_mask
    )
    preds = np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    ...
    before_time = datetime.now()
    _, preds, labels = compute_mlps_importance(
        args, model, eval_dataloader, compute_importance=False, mlp_mask=None
    )
    preds = np.argmax(preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
...
def main():
    ...
    if args.try_masking and args.masking_threshold > 0.0 and args.masking_threshold < 1.0:
        mlp_mask = mask_mlps(args, model, eval_dataloader)
        prune_heads(args, model, eval_dataloader, mlp_mask)
    else:
        #Compute head entropy and importance score
        compute_mlps_importance(args, model, eval_dataloader)
