    ...
    if labels_filename is not None:
        with open(labels_filename, 'r') as f:
            for line in f:
                ...
                label, images_names_str = [x.strip() for x in line.split('=')]

                # merge `{single|double|multiple}_vertical` labels
                if 'vertical' in label:
                    label = 'vertical'

                images_names = [standartize_image_name(x) for x in images_names_str.split(',')]

                labels_dict.update({image_name: label for image_name in images_names})
